...
public class commonFunctions {
	...
	/*public static void readMarkdownFile(String fileName) {
		try {
			Parser parser = new Parser(System.getProperty("user.dir")+"/src/test/resources/testdata/GE-profile-smart-range.md");
			Markdown = new FormattedTextOptions(FormattedTextMode.PlainText);
			TextReader reader = parser.getText(new TextOptions(true));
	...
	/*public static void convertMDtoJSON(String fileName) {
		Converter converter = new Converter(System.getProperty("user.dir")+"/src/test/resources/testdata/GE-profile-smart-range.md");
		// Prepare conversion options for target format 
						...
						if(title.contains("List"))
						{
							...
							for(int x=1; x<=splitList.length; x++) {
								...
								for(int k=1;k<dump.length;k++) {
									...
									String sp_char2 = "+";
									String sp_char3 = "~";
									if(linkVal.contains(sp_char) || linkVal.contains(sp_char2) || linkVal.contains(sp_char3)) {
										value = value.replaceAll("\\?", "");
